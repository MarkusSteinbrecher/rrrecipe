...
function icon(name: string, size = 14): string {
  ...
  return `<svg aria-hidden="true" width="${size}" height="${size}" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6" fill="currentColor">${paths[name] ?? ""}</svg>`;
}
...
function formatScaledQuantity(ingredient: IngredientLine, multiplier: number): string {
  const quantity = parseQuantity(ingredient.quantity);
  if (quantity === undefined) return ingredient.quantity && ingredient.unit ? `${ingredient.quantity} ${ingredient.unit}` : ingredient.quantity ?? "";
